...
require './defenses/all_defenses.rb'

class Level10
  def self.machine_turn game, turn
    puts "Joe level 10 plays"

    return if Fatality.execute game
    
    return if turn == 0 && (AdvancedAttacks.set game)   

    return if turn == 1 && (MediumDefense.avoid_basic_traps game)  

    if turn == 2
      return if Conner.first_step game
      return if V.first_step game
      return if Boomerang.first_step game
      return if GodLevelConner.first_step game
    end

    return if turn == 3 && (AdvancedDefence.avoid_advanced_traps game)
